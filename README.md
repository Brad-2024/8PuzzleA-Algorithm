...
The effective branching factors stay close to 1 (1.27-1.50 across all three heuristics, which by R&N's definition make the tested heuristics reasonably good.

For every tested solution depth d, Gh<sub>2</sub>(d) ≤ Gh<sub>3</sub>(d) ≤ Gh<sub>1</sub>(d), where Gh(d) is the number of nodes generated by A* using heuristic h at depth d (e.g. at d=28, h<sub>2</sub> generates roughly 10x fewer nodes than either h<sub>1</sub> or h<sub>3</sub>).

Although Manhattan distance takes longer to compute per node than h<sub>1</sub>, the reduction in nodes expanded far outweighs that per-node overhead.
